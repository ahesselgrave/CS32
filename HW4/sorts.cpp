...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    {
        for(int sortSize = 2; sortSize <= s.size(); sortSize++)
        {
            if (comp(s[sortSize - 1], s[sortSize - 2]))
            {
...
void report(string caption, double t, const vector<Store>& s)
{
    ...
    char* captionC = new char[caption.size() + 1];
    for (int i = 0; i < caption.size(); i++)
        captionC[i] = caption[i];
    captionC[caption.size()] = '\0';
    printf("\033[38;2;0;255;0m%lf milliseconds; %s; first few stores are\n  \033[0;00m", t, captionC);
    delete captionC;
    size_t n = s.size();
    if (n > 5)
        n = 5;
    for (size_t k = 0; k < n; k++)
        printf("\033[38;2;30;30;255m (%d, %lf)\033[0;00m", s[k].id, s[k].total);
...
int main()
{
      ...
      // destruction time.
    {
        ...
        for (vector<Store>::iterator itr = auxStores.begin(); itr != auxStores.end(); itr++)
            auxStorePtrs.push_back(&(*itr));
      
        sort(auxStorePtrs.begin(), auxStorePtrs.end(), compareStorePtr);
